    ...
    T: SizedSample + FromSample<f32>,
{
    for frame in output.chunks_mut(2) {
        ...
        // Convert f32 samples to i16 (WAV files typically use 16-bit integer samples)
        let left_sample = (left * i16::MAX as f32) as i16;
        let right_sample = (right * i16::MAX as f32) as i16;

        // Assign the samples to the frame
        frame[0] = T::from_sample(left_sample as f32);
        frame[1] = T::from_sample(right_sample as f32);

        // Write the stereo frame to the WAV file
        writer
