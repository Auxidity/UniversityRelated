/*
* READ THIS:
* This is a working pitch shifter (albeit on startup it does underflow). I did add a catch-up block
* before it outputs anything but its currently in wrong place (too late). It does however add a
* permanent delay of ~2 seconds with current threshhold.
*
* It is utilizing a c++ wrapper for a pitch shifting library (Im only handling raw streams..).
* ToDo: add RTP streaming to a playback device, then add RTP streaming to a virtual mic. Then cross
* compile to RasPi.
* */
